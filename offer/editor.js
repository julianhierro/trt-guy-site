...
(function () {
  ...
  // ── jv-challenge-page add-ons ────────────────────────────────────────────
  // 1. Double-click any image to replace it (upload or paste a URL).
  // 2. Double-click a video to swap it for another YouTube / Vimeo link.
  // 3. Every buy button (.cta) and price line (.price-note) stays identical:
  //    edit the text or the link of one and the rest follow.
  function syncCtaAttrs(el) {
    var cta = el && el.closest && el.closest(".cta");
    if (!cta) return;
    document.querySelectorAll(".cta").forEach(function (o) {
      if (o === cta) return;
      ["href", "target", "rel"].forEach(function (k) {
        if (cta.hasAttribute(k)) o.setAttribute(k, cta.getAttribute(k)); else o.removeAttribute(k);
      });
      markAttr(o);
    });
  }
  document.addEventListener("input", function (e) {
    if (!editMode || !e.target.closest) return;
    var src = e.target.closest(".cta, .price-note");
    if (!src) return;
    var sel = src.classList.contains("cta") ? ".cta" : ".price-note";
    document.querySelectorAll(sel).forEach(function (o) { if (o !== src) o.innerHTML = src.innerHTML; });
  });
  document.addEventListener("dblclick", function (e) {
    if (!editMode || !e.target.closest) return;
    if (e.target.closest(".jv-toolbar, .jv-launcher, .jv-section-tools")) return;
    var vid = e.target.closest(".vid");
    if (vid) {
      e.preventDefault(); e.stopPropagation();
      var fr = vid.querySelector("iframe"); if (!fr) return;
      var u = prompt("Paste the new YouTube or Vimeo link:", (fr.getAttribute("src") || "").replace("about:blank", ""));
      if (!u) return;
      var emb = toEmbed(u); if (!emb) { alert("Couldn't read that video link."); return; }
      pushUndo();
      fr.setAttribute("src", emb); markAttr(fr); scheduleDraft();
      flash("Video swapped — Publish to make it live");
      return;
    }
    ...
    chooseImage(function (url) {
      if (!url) return;
      pushUndo();
      img.setAttribute("src", url); img.removeAttribute("srcset");
      markAttr(img); scheduleDraft();
